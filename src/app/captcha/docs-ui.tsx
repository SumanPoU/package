        ...
        <aside className="sticky top-20 hidden h-[calc(100vh-6rem)] w-52 shrink-0 overflow-y-auto lg:block xl:w-56">
          <nav aria-label="Documentation" className="flex flex-col gap-0.5">
            ...
            <div className="mb-4 flex flex-col gap-0.5 px-2.5">
              ...
              <p className="pkg text-[11px]">@itzsa/captcha</p>
            </div>
            {DOC_NAV.map((item) => (
              <NavLink
                key={item.id}
...
export function PropsTable({
  ...
  return (
    <div className="overflow-x-auto rounded-md border-[0.5px] border-border bg-white dark:bg-card">
      {caption ? (
        <p className="border-b-[0.5px] border-border bg-white px-3 py-2 font-mono text-[11px] tracking-wide text-tertiary uppercase dark:bg-card">
          {caption}
        </p>
      ) : null}
      <table className="w-full min-w-[40rem] border-collapse bg-white text-left text-sm dark:bg-card">
        <thead>
          <tr className="border-b-[0.5px] border-border bg-white dark:bg-card">
            <th className="bg-white px-3 py-2.5 text-[12px] font-medium text-primary dark:bg-card">
              Prop
            </th>
            <th className="bg-white px-3 py-2.5 text-[12px] font-medium text-primary dark:bg-card">
              Type
        ...
        <tbody className="bg-white dark:bg-card">
          {rows.map((row) => (
            <tr
              key={row.name}
              className="border-b-[0.5px] border-border bg-white last:border-0 dark:bg-card"
            >
              <td className="bg-white px-3 py-2.5 align-top font-mono text-[12.5px] text-accent dark:bg-card">
                {row.name}
              </td>
              <td className="max-w-[14rem] bg-white px-3 py-2.5 align-top font-mono text-[11.5px] leading-snug break-all text-secondary dark:bg-card">
                {row.type}
              </td>
              <td className="bg-white px-3 py-2.5 align-top font-mono text-[12px] whitespace-nowrap text-tertiary dark:bg-card">
                {row.default ?? "—"}
              </td>
              <td className="bg-white px-3 py-2.5 align-top text-[13px] leading-relaxed text-secondary dark:bg-card">
                {row.description}
              </td>
            </tr>
          ))}
