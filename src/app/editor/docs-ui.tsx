...
export function DocsShell({ children }: { children: ReactNode }) {
  return (
    <PackageDocsShell title="Editor" packageName="@itzsa/editor" nav={DOC_NAV}>
      {children}
    </PackageDocsShell>
  );
}
  ...
  return (
    <section id={id} className="scroll-mt-28 flex flex-col gap-4">
      <header className="flex flex-col gap-1.5">
        <Heading
          className={cn(
            "group scroll-mt-28 font-medium tracking-tight text-primary",
            level === 3 ? "text-base" : "text-lg sm:text-xl",
          )}
        >
          <a href={`#${id}`} className="no-underline hover:text-accent">
            {title}
            <span className="ml-1.5 text-tertiary opacity-0 transition-opacity group-hover:opacity-100">
              #
            </span>
          </a>
        </Heading>
